...
//            throw new Exception("Erro, players deve ser>0");
        ...
        float base=Track.TRACKWIDTH+Track.TRACKWIDTH/3;
        for(int i=0;i<players;i++){
            float Xoffset=i*base;
            WTGame wttmp=new WTGame(game,t,Xoffset);
            screens.add(wttmp);
        }
        MiddleOffset=base*(players-1)/2.0f;
        ...
        if(!loading){
            if(MultipleScreens || screens.size()==1){
                ...
                switch(screens.size()){
                    ...
                    //B
//                    case 4:
//                        sizex=(int)Math.ceil(Gdx.graphics.getWidth()/4);
//                        sizey=(int)Gdx.graphics.getHeight();
//                        Gdx.gl.glViewport(0,0,sizex,sizey);
//                        renderScreenById(0,delta);
//                        Gdx.gl.glViewport((int)Math.floor(Gdx.graphics.getWidth()/4),0,sizex,sizey);
//                        renderScreenById(1,delta);
//                        Gdx.gl.glViewport((int)Math.floor(Gdx.graphics.getWidth()*2/4),0,sizex,sizey);
//                        renderScreenById(2,delta);
//                        Gdx.gl.glViewport((int)Math.floor(Gdx.graphics.getWidth()*3/4),0,sizex,sizey);
//                        renderScreenById(3,delta);
//                        break;
                }
            }else{
            ...
            if(!started){
                ...
                if(FPScount>=WhippingTopGame.FPS){
                    if(FPScount==WhippingTopGame.FPS)
                        soundTop0.play();
                    semRed.draw(batch);
                }
                if(FPScount>=2*WhippingTopGame.FPS){
                    if(FPScount==2*WhippingTopGame.FPS)
                        soundTop0.play();
                    semYellow.draw(batch);
                }
                if(FPScount>=3*WhippingTopGame.FPS){
                    if(FPScount==3*WhippingTopGame.FPS)
                        soundTop1.play();
                    semGreen.draw(batch);
                }
                if(FPScount>=3.1f*WhippingTopGame.FPS){
                    started=true;
                }
                batch.end();
            }else{
    ...
    public void configCam(){
        ...
        cam.position.set(MiddleOffset, 40f, -3f);
