...
public class ModelCreator {
    ...
    private static double[] createTrackModel_createFloor(double size,double pos, double Xoffset, int widthReducer,float depth,boolean defaultUVrange,Material mat,ModelBuilder modelBuilder){
        if(size>0){
            ...
            base.rect(  (float)(-Track.TRACKWIDTH/widthReducer+Xoffset),depth,(float)pos, 
                        (float)(-Track.TRACKWIDTH/widthReducer+Xoffset),depth,(float)(size+pos), 
                        (float)(Track.TRACKWIDTH/widthReducer+Xoffset),depth,(float)(size+pos), 
                        (float)(Track.TRACKWIDTH/widthReducer+Xoffset),depth,(float)pos,               0, 1, 0);
        }
        pos+=size;
        size=0;
    ...
    public static ModelInstance createTrackModel(Track track,double Xoffset,Texture floorText, Texture arrivalText, Texture[] bridges,Texture speedUpText){
        ...
        for(int i=0;i<track.getFullSize();i++){           
            if(track.obstacleTypeAt(i)==Obstacle.BRIDGE){
                ...
                lastBridge=i;
                size=createTrackModel_createFloor(trackRealSize,movedPosZ,Xoffset,2,0,false,trackMaterial,modelBuilder);
                trackRealSize=size[0];
                movedPosZ=size[1];
                size=createTrackModel_createFloor(track.sizeAt(i),movedPosZ,Xoffset,8,0,true,obstacleMaterial,modelBuilder);
                trackRealSize=size[0];
                movedPosZ=size[1];
            }else{
                if(track.obstacleTypeAt(i)==Obstacle.SPEEDUP){
                    obstacleMaterial = new Material(TextureAttribute.createDiffuse(speedUpText));
                    createTrackModel_createFloor(track.sizeAt(i),movedPosZ+trackRealSize,Xoffset,2,0.1f,true,obstacleMaterial,modelBuilder);
        ...
        createTrackModel_createFloor(trackRealSize,movedPosZ,Xoffset,2,0,false,trackMaterial,modelBuilder);
        return new ModelInstance(modelBuilder.end());
    }
    
    public static List<ModelInstance> createTrackObjs(Track track,double Xoffset,Model grassModel,Model[] cones){
