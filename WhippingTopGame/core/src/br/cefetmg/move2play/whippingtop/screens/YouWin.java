...
public class YouWin implements Screen,Move2PlayGame{
    ...
    public YouWin(WhippingTopGame game){
        this.game=game;
        candyFont=Util.createBitMapFromTTF(Fonts.Candy(), Color.BLUE, 35);
    ...
    public void draw(float delta){
        ...
        glyphLayout.setText(candyFont, "You Win");
        float textSizeX=glyphLayout.width/2;
        float textSizeY=glyphLayout.height/2;
        batch.begin();
        candyFont.draw(batch, "You Win", Gdx.graphics.getWidth()/2-textSizeX, Gdx.graphics.getHeight()/2+textSizeY);
        batch.end();
    }
